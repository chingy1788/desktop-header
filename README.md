...
- **Per-Desktop Sticky Notes & Responsive Collapsing**: Includes an expandable notes dropdown on the right side of the screen. When collapsed, it shows a low-profile preview of the first line. Clicking it expands a beautiful dark glassmorphic text editor that auto-saves your notes per virtual desktop based on their unique COM GUIDs. **If horizontal screen space becomes too tight (e.g. many or long desktop names), the notes panel automatically collapses into a sleek `📝` button. Clicking the button opens a premium floating glassmorphic popup editor to ensure fully accessible and distraction-free workspace notes.**
...
- **Dev Environment Spawner**: Allows spawning a complete custom development workspace (PowerShell terminal, Visual Studio Code, GitExtensions, and Windows File Explorer) all opened at `C:\dev\<headername>` (where `<headername>` is the sanitized active virtual desktop name) with a single click from the tray menu. The menu item automatically updates its label dynamically to show the active virtual desktop name, sanitizes folder names safely, and handles missing or uninstalled applications gracefully without blocking others or crashing the overlay.
